...
//                        println!("===={:?}", mut_anti1.less_equal(&time));
                        if edge_cap1.time().first <= time {
                            let new_pair = Pair::new(time, edge_cap1.time().second);
                            edge_input1
                    ...
                    InputChoice::Input2(data, time, diff) => {
                        if edge_cap2.time().second <= time {
                            let new_pair = Pair::new(edge_cap2.time().first, time);
                            edge_input2
                    ...
                    InputChoice::Frontier1(time) => {
                        ...
                        let pair_time = Pair::new(time, edge_cap1.time().second);
                        edge_cap1.downgrade(&pair_time);
                        edge_cap2.downgrade(&pair_time);
                    },
                    InputChoice::Frontier2(time) => {
                        let pair_time = Pair::new(edge_cap2.time().first, time);
                        edge_cap1.downgrade(&pair_time);
                        edge_cap2.downgrade(&pair_time);
                    }
                };
